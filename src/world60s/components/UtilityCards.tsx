      ...
      <div className="w60-douban-grid">
        {data?.slice(0, 6).map((item) => (
          <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer" className="w60-douban-item">
            <img src={item.cover_proxy || item.cover} alt={item.title} className="w60-douban-cover" />
            <div className="w60-douban-mask" />
            <div className="w60-douban-info">
              <div className="w60-douban-title">{item.title}</div>
              <div className="w60-douban-rating">{item.rating}</div>
            </div>
          </a>
        ))}
      </div>
